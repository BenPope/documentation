...
Simple Authentication Security Layer (SASL) is a framework that lets the user choose the authentication mechanism. The mechanism determines which authentication flow the client uses for that user/password. 

Redpanda supports multiple authentication mechanisms: GSSAPI (Kerberos) and SCRAM (Salted Challenge Response Authentication Mechanism). SCRAM provides strong encryption for user names and passwords by default and does not require an external data store for user information. Redpanda supports  `SCRAM-SHA-256` and `SCRAM-SHA-512`.
...
:::info
Kerberos authentication requires an [Enterprise license](../../introduction/licenses). To upgrade, contact [Redpanda sales](https://redpanda.com/try-redpanda?section=enterprise-cloud).
:::
...
1. Ensure that host names and fully qualified domain names (FQDN) are configured properly.

2. Configure the `/etc/krb5.conf` [Kerberos configuration file](http://web.mit.edu/Kerberos/krb5-latest/doc/admin/conf_files/krb5_conf.html) to use Active Directory or another corporate key distribution center (KDC). In the following `krb5.conf` example, realm information points to the corporate KDC.
  ...
  [libdefaults]
   default_realm    = EXAMPLE.REALM.COM
  ...
  [realms]
   EXAMPLE.REALM.COM = {
    kdc          = WIN-SRV1.EXAMPLE.REALM.COM
    admin_server = WIN-SRV1.EXAMPLE.REALM.COM
   }
  [domain_realm]
   .demo = EXAMPLE.REALM.COM 
    demo = EXAMPLE.REALM.COM
  ```

3. Set `sasl_kerberos_principal_mapping`. This maps external users onto Redpanda principals used in the ACLs.

  By default Redpanda matches the primary of the Kerberos user principal (UPN) of the user.
  ...
  Each rule has the following format:
    ...
    * `RULE:[n:string](regexp)s/pattern/replacement/g/c`. Where:
      ...
      * If this matches, then a string will be formed from `string`, substituting the realm of the principal for `$0` and the `n`’th component of the principal for `$n` (e.g., if the principal was `johndoe/admin` then `[2:$2$1foo]` would result in the string `adminjohndoefoo`).
      ...
      * `/c` is optional and can be either `/L` or `/U` to make the match lowercase or uppercase.
  ...
  Some examples for a Kerberos UPN without a host (`jdoe`) and with a host (`jdoe/host`):

  | Translation | jdoe | jdoe/host |
  | -- | -- | -- |
  ...
  ¹ Rule does not match because there are two components in principal name `jdoe/host@YOUR-REALM.COM`

  ² Rule does not match because there is one component in principal name `jdoe@YOUR-REALM.COM`

  The first rule that matches is used to extract a principal.

  To get the `sasl_kerberos_principal_mapping` property, run:

  ```bash
  rpk cluster config get sasl_kerberos_principal_mapping
  ```

  To set the `sasl_kerberos_principal_mapping` property, run:

  ```bash
  rpk cluster config set sasl_kerberos_principal_mapping '["RULE:[1:$1@$0](.*@MYDOMAIN.COM)s/@.*//","DEFAULT"]'
  ```

4. For each node, create the principal `redpanda/<FQDN>@<REALM>`, ensuring that the expiration date is long enough and that the ticket is renewable.

#### Enable Kerberos

You configure Kerberos authentication using a keytab, which contains the user credentials. 

1. Define the location of the Redpanda keytab file. This must be located at an identical file path on all Redpanda nodes. Default is `/var/lib/redpanda/redpanda.keytab`.

  ```bash   
  $ rpk cluster config set sasl_kerberos_keytab <path>
  ```

2. Define the primary of the service principal to be used by Redpanda with the given keytab. Default is `redpanda`.
  
  ```bash
  $ rpk cluster config set sasl_kerberos_principal <name>
  ```

3. Append the list of allowed SASL mechanisms that clients can use to authenticate against the Kafka API. For example, if Redpanda currently supports only SCRAM then the configuration may return the following:
...
4. To add support for Kerberos, append the `sasl_mechanisms` property with the value `GSSAPI`:

  ```bash
  $ rpk cluster config set sasl_mechanisms '["SCRAM","GSSAPI"]'
  ```

5. Enable SASL, if not already enabled:

  ```bash
  $ rpk cluster config set kafka_enable_authorization true
  ```

6. If authentication is being configured for the first time, you may need to configure authorization [ACLs](../../security/authorization#acls) before users can access Redpanda resources.
