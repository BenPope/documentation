...
2. Install the `krb5-workstation` library on all Redpanda nodes. For example, on RHEL/CentOS:
   
  ```bash
  $ sudo yum install krb5-workstation krb5-libs
  ```

3. Install required libraries for Kerberos on Redpanda nodes. Configure the `/etc/krb5.conf` file to use Active Directory or another corporate key distribution center (KDC). 
   
   In the following example, `krb5.conf` shows realm information to point to the corporate KDC.
...
1. To add support for Kerberos, append the `sasl_mechanisms` property, either the value `GSSAPI`:
