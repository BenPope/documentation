...
4. For each node, create the principal `redpanda/<FQDN>@<REALM>.COM`, ensuring that the expiration date is long enough and that the ticket is renewable.
...
2. Define the service principal to be used by Redpanda with the given keytab. Default is `redpanda`.
