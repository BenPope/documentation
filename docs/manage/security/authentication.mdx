...
Configure Kerberos authentication using a keytab, which contains the user credentials. This requires generating and verifying the keytab. 
...
1. For each node, create two principals: `host/<FQDN>@<REALM>.COM` and `redpanda/<FQDN>@<REALM>.COM`, ensuring that the expiration date is long enough and that the tickets are renewable.

#### Enable Kerberos

1. Generate keytabs for all (or any new) nodes:
