...
3. Set `sasl_kerberos_principal_mapping`. This maps external users onto Redpanda principals used in the ACLs.
  ...
  For information about the rules, see the [Kafka documentation](https://kafka.apache.org/documentation/#security_authz_sasl).
...
1. To add support for Kerberos, append the `sasl_mechanisms` property, either with the value `GSSAPI`:

  ```bash
  $ rpk cluster config set sasl_mechanisms '["SCRAM","GSSAPI"]'
  ```

  or with its synonym value `KERBEROS`:

  ```bash
  $ rpk cluster config set sasl_mechanisms '["SCRAM","KERBEROS"]'
  ```
